  ...
  if ($('.scroll__link').length > 0) {
    ...
    var setCurrentItem = function() {
      ...
      $('.scroll__link')
        ...
        .blur()
      $(".scroll__link[href='#" + current + "']").addClass(
        'scroll__link--active'
      )
    }
    setCurrentItem()
  ...
  if ($('.sidebar__section--sticky').length > 0) {
    ...
    var setSidebarPosition = function() {
      if ($(window).scrollTop() > sidebarOffset.top - 48) {
        $('.sidebar__section--sticky').addClass(
          'sidebar__section--sticky--fixed'
        )
      } else {
        $('.sidebar__section--sticky').removeClass(
          'sidebar__section--sticky--fixed'
        )
      }
    }
